...
function playersWithProperty(req, res, next) {
    db.many(`
        SELECT name, emailAddress, propertyName FROM Player, Property, Game
        WHERE Property.gameID = Game.id
        AND Property.playerID = Player.id
        ;`
    ).then(data => {
        res.send(data);
    }).catch(err => {
